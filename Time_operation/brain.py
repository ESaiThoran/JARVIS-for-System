...
def parse_input(input_text):
    ...
    if times:
        ...
        formatted_output = f"{formatted_time} = Sir its time to reminder you, {updated_input_text}"
...
def input_manage(input_text):
        ...
        if output != "No valid time found in input":
            save_to_file(output, time, r"D:\A.J.A.R.V.I.S\schedule.txt")
...
def input_manage_Alam(input_text):
        ...
        if time != "No valid time found in input":
            save_to_Alarmfile(time, r'D:\A.J.A.R.V.I.S\Alam_data.txt')
            print("Schedule Saved")
        else:
            print("pass")
